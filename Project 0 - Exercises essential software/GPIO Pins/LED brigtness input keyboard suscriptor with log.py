	...
	try:
		if("| " in messageStr):
			...
			for pinNum in pinNums:
				...
				logFile.write("{}~{}~{}~{}\n".format(datetime.datetime.now(), messageStr, message.topic, "Encendiendo LED {} a {}% duty".format(pinNum, percentage)))
		...
		logFile=open("log suscriptor.txt", "a", encoding="utf8")
		logFile.write("{}~{}~{}~{}\n".format(datetime.datetime.now(), messageStr, message.topic, "Error al ejecutar el mensaje recibido - {} line {}".format(e, sys.exc_info()[-1].tb_lineno)))
		logFile.close()
