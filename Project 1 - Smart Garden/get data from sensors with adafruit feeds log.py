...
def main():
    ...
    logFile=open("log get.txt", "a", encoding="utf8")
    logFile.write("{}~{}~{}~{}~{}\n".format(datetime.datetime.now(), "Nulo", "Nulo", "Aplicación iniciada", "Nulo"))
    logFile.close()
	...
	while True:
		...
		if(soilMoistureData.updated_at!=soilMoistureLastUpdate):	
			...
			if(state!=soilMoistureLastState):
				...
				if(state=="humedad suelo baja"):
					aio.send(irrigationFeed.key, "ON")
				else:
					aio.send(irrigationFeed.key, "OFF")
			
			soilMoistureLastUpdate=soilMoistureData.updated_at
			

            logFile=open("log get.txt", "a", encoding="utf8")
            logFile.write("{}~{}~{}~{}~{}\n".format(datetime.datetime.now(), soilMoistureFeed.key, "Humedad del suelo", "recibido - {}".format(), state))
            logFile.close()
		...
		if(ambientTemperatureData.updated_at!=temperatureLastUpdate):
            ...
            logFile=open("log get.txt", "a", encoding="utf8")
            logFile.write("{}~{}~{}~{}~{}\n".format(datetime.datetime.now(), soilMoistureFeed.key, "Temperatura del ambiente", "recibido - {}".format(), state))
            logFile.close()
		...
		if(ambientHumidityData.updated_at!=humidityLastUpdate):
            ...
            logFile=open("log get.txt", "a", encoding="utf8")
            logFile.write("{}~{}~{}~{}~{}\n".format(datetime.datetime.now(), soilMoistureFeed.key, "Humedad del ambiente", "recibido - {}".format(), state))
            logFile.close()
