...
if __name__=="__main__":
	...
	# Write on log file
	logFile=open("log send.txt", "a", encoding="utf8")
	...
	while True:
		...
		logFile=open("log send.txt", "a", encoding="utf8")
		logFile.write("{}~{}~{}~{}\n".format(datetime.datetime.now(), soilMoistureFeed.key, "Humedad Suelo", "enviado - {}".format(soilMoisturePercentage)))
		logFile.close()
		...
		if humidity is not None and temperature is not None:
			...
			logFile=open("log send.txt", "a", encoding="utf8")
			logFile.write("{}~{}~{}~{}\n".format(datetime.datetime.now(), ambientTemperatureFeed.key, "Temperatura del ambiente", "enviado - {}".format(temperature)))
			logFile.close()

			aio.send(ambientHumidityFeed.key, float(humidity))

			logFile=open("log send.txt", "a", encoding="utf8")
			logFile.write("{}~{}~{}~{}\n".format(datetime.datetime.now(), ambientHumidityFeed.key, "Humedad del ambiente", "enviado - {}".format(humidity)))
			logFile.close()
